...
class StartController(object):
    ...
    def _default(self, container_uuid):
        ...
        LOG.debug('Calling rpcapi.container_start with %s' %
                  container_uuid)
...
class StopController(object):
    ...
    def _default(self, container_uuid, *remainder):
        if pecan.request.method != 'PUT':
            pecan.abort(405, ('HTTP method %s is not allowed'
                              % pecan.request.method))
        LOG.debug('Calling rpcapi.container_stop with %s' %
                  container_uuid)
...
class RebootController(object):
    ...
    def _default(self, container_uuid, *remainder):
        if pecan.request.method != 'PUT':
            pecan.abort(405, ('HTTP method %s is not allowed'
                              % pecan.request.method))
        LOG.debug('Calling rpcapi.container_reboot with %s' %
                  container_uuid)
...
class PauseController(object):
    ...
    def _default(self, container_uuid, *remainder):
        if pecan.request.method != 'PUT':
            pecan.abort(405, ('HTTP method %s is not allowed'
                              % pecan.request.method))
        LOG.debug('Calling rpcapi.container_pause with %s' %
                  container_uuid)
...
class UnpauseController(object):
    ...
    def _default(self, container_uuid, *remainder):
        if pecan.request.method != 'PUT':
            pecan.abort(405, ('HTTP method %s is not allowed'
                              % pecan.request.method))
        LOG.debug('Calling rpcapi.container_unpause with %s' %
                  container_uuid)
...
class LogsController(object):
    ...
    def _default(self, container_uuid, *remainder):
        if pecan.request.method != 'GET':
            pecan.abort(405, ('HTTP method %s is not allowed'
                              % pecan.request.method))
        LOG.debug('Calling rpcapi.container_logs with %s' %
        container_uuid)
...
class ExecuteController(object):
    ...
    def _default(self, container_uuid, command, *remainder):
        if pecan.request.method != 'PUT':
            pecan.abort(405, ('HTTP method %s is not allowed'
                              % pecan.request.method))
        LOG.debug('Calling rpcapi.container_execute with %s command %s'
...
class ContainersController(rest.RestController):
    ...
    def post(self, container):
        ...
        res_container = rpcapi.container_create(new_container.name,
                                                new_container.uuid,
                                                new_container)
