...
from magnum.conductor import bay_definition
from magnum.tests import base


class BayDefinitionTestcase(base.TestCase):
    def test_get_bay_definitions(self):
        bay_defs = bay_definition.get_bay_definitions()

        vm_atomic_k8s = bay_defs[('vm', 'fedora-atomic', 'kubernetes')]
        vm_coreos_k8s = bay_defs[('vm', 'coreos', 'kubernetes')]

        self.assertEqual(len(vm_atomic_k8s), 1)
        self.assertEqual(vm_atomic_k8s['magnum_vm_atomic_k8s'],
                         bay_definition.K8sBayDefinition)
        self.assertEqual(len(vm_coreos_k8s), 1)
        self.assertEqual(vm_coreos_k8s['magnum_vm_coreos_k8s'],
                         bay_definition.CoreOSK8sBayDefinition)

    def test_get_vm_atomic_kubernetes_definition(self):
        bay_def = bay_definition.get_bay_definition('vm', 'fedora-atomic',
                                                    'kubernetes')

        self.assertIsInstance(bay_def,
                              bay_definition.K8sBayDefinition)

    def test_get_vm_coreos_kubernetes_definition(self):
        bay_def = bay_definition.get_bay_definition('vm', 'coreos',
                                                    'kubernetes')
    ...
    def test_get_definition_not_supported(self):
        self.assertRaises(exception.BayTypeNotSupported,
                          bay_definition.get_bay_definition,
    ...
    def test_required_param_not_set(self):
        param = bay_definition.ParameterMapping('test', baymodel_attr='test',
                                                required=True)
