...
cfg.CONF.register_opts(bay_def_opts, group='bay')

DEFINITION_MAP = None


def get_bay_definitions():
    '''Retrieves bay definitions from python entry_points.

    Example:

    With the following classes:
    class BayDefinition1(BayDefinition):
        provides = [
            ('platform1', 'os1', 'coe1')
        ]

    class BayDefinition2(BayDefinition):
        provides = [
            ('platform2', 'os2', 'coe2')
        ]
    ...
    magnum.bay_definitions =
        bay_def_1_name = some.python.path:BayDefinition1
        bay_def_2_name = some.python.path:BayDefinition2

    get_bay_definitions will return:
        {
            (platform1, os1, coe1): {'bay_def_1_name': BayDefinition1},
            (platform2, os2, coe2): {'bay_def_2_name': BayDefinition1}
        }

    :return: dict
    '''

    global DEFINITION_MAP

    if not DEFINITION_MAP:
        DEFINITION_MAP = dict()
        for entry_point in iter_entry_points('magnum.bay_definitions'):
            cls = entry_point.load()
            for bay_type in cls.provides:
                bay_type_tuple = (bay_type['platform'],
                                  bay_type['os'],
                                  bay_type['coe'])
                providers = DEFINITION_MAP.get(bay_type_tuple, dict())
                providers[entry_point.name] = cls
                DEFINITION_MAP[bay_type_tuple] = providers

    return DEFINITION_MAP


def get_bay_definition(platform, os, coe):
    '''Returns the enabled BayDefinition class for the provided bay_type.
    ...
    get_bay_definition('platform2', 'os2', 'coe2') will return: BayDefinition2
    ...
    :param coe: The Container Orchestration Environment the bay will produce

    :return: class
    '''

    definition_map = get_bay_definitions()
    bay_type = (platform, os, coe)

    if bay_type not in definition_map:
        raise exception.BayTypeNotSupported(platform=platform, os=os, coe=coe)
    definitons_for_type = definition_map[bay_type]

    for name in cfg.CONF.bay.enabled_definitions:
        if name in definitons_for_type:
            return definitons_for_type[name]()
...
class OutputMapping(object):
    ...
    def __init__(self, name, heat_output):
        self.name = name
        self.heat_output = heat_output

    def set_output(self, outputs, stack):
        for output in stack.outputs:
            if output['output_key'] == self.heat_output:
                outputs[self.name] = output['output_value']
                break


@six.add_metaclass(abc.ABCMeta)
class BayDefinition(object):
    '''A BayDefinition is essentially a mapping between Magnum objects
    and Heat templates. Each BayDefinition has a mapping of Heat parameters
    '''
    ...
    def add_output(self, name, heat_output):
        output = OutputMapping(name, heat_output)
    ...
    def get_outputs(self, stack):
        ...
        for output in self.output_mappings:
            output.set_output(template_outputs, stack)
...
class K8sBayDefinition(BayDefinition):
    provides = [
        {'platform': 'vm', 'os': 'fedora-atomic', 'coe': 'kubernetes'},
    ]

    def __init__(self):
        super(K8sBayDefinition, self).__init__()
        ...
        self.add_output('api_address', 'kube_master')
        self.add_output('minions_address', 'kube_minions_external')
        self.add_output('kube_minions', 'kube_minions')

    def template_path(self):
        return cfg.CONF.bay.k8s_atomic_template_path


class CoreOSK8sBayDefinition(K8sBayDefinition):
    provides = [
        {'platform': 'vm', 'os': 'coreos', 'coe': 'kubernetes'},
    ]

    def __init__(self):
        super(CoreOSK8sBayDefinition, self).__init__()
    ...
    def get_params(self, baymodel, bay, extra_params=None):
        ...
        return super(CoreOSK8sBayDefinition,
