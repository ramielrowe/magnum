...
def _extract_template_definition(context, bay):
    ...
    defintiion = template_definition.get_template_definition('vm',
                                                cfg.CONF.k8s_heat.cluster_type,
                                                'kubernetes')
    return defintiion.template_path(), defintiion.get_params(baymodel, bay)
...
def _parse_stack_outputs(stack):
    definition = template_definition.get_template_definition('vm',
                                                cfg.CONF.k8s_heat.cluster_type,
                                                'kubernetes')
    return definition.get_outputs(stack)
...
class HeatPoller(object):
    ...
    def poll_and_check(self):
        ...
        if (stack.stack_status in ['CREATE_COMPLETE', 'UPDATE_COMPLETE']):
            ...
            self.bay.node_addresses = parsed_outputs["node_addresses"]
