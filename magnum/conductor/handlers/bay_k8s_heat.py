...
def _extract_bay_definition(context, bay):
    ...
    bay_def = bay_definition.get_bay_definition('vm',
                                                cfg.CONF.k8s_heat.cluster_type,
                                                'kubernetes')
    return bay_def.template_path(), bay_def.get_params(baymodel, bay)


def _create_stack(context, osc, bay):
    template_path, heat_params = _extract_bay_definition(context, bay)
...
def _parse_stack_outputs(stack):
    bay_def = bay_definition.get_bay_definition('vm',
                                                cfg.CONF.k8s_heat.cluster_type,
                                                'kubernetes')
    return bay_def.get_outputs(stack)
...
class HeatPoller(object):
    ...
    def poll_and_check(self):
        ...
        if (stack.stack_status in ['CREATE_COMPLETE', 'UPDATE_COMPLETE']):
            parsed_outputs = _parse_stack_outputs(stack)
            self.bay.api_address = parsed_outputs["api_address"]
            self.bay.minions_address = parsed_outputs["minions_address"]
